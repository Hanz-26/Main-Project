...
const QUIZ_SCENE = preload("res://scenes/dionny_quiz_popup.tscn")

func _ready() -> void:
	add_to_group("boss")
	_original_modulate = animated_sprite.modulate
	if self.name == "BossSlimeRed":
		self.get_node("AnimatedSprite2D").self_modulate = Color(1, 0.25, 0.25, 1)
...
func _on_quiz_completed() -> void:
	...
	self.get_node("EncounterZone").monitoring = false
	self.get_node("EncounterZone").set_deferred("monitorable", false)
...
func enemy_take_damage():
	...
	if self.name == "BossSlimeRed":
		self.get_node("AnimatedSprite2D").modulate = Color(1, 0.25, 0.25, 1)
	if _health <= 0:
		_die()
	else:
