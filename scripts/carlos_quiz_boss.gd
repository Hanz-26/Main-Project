...
func show_quiz():
	quiz_active = true
	get_tree().paused = true# This pauses the game for the quiz
	...
	#
	quiz_ui.process_mode = Node.PROCESS_MODE_ALWAYS# This enables the quiz to run while the game is paused
	#
...
func close_quiz():
	quiz_active = false
	if game_manager.lives > 0 and game_manager.hearts > 0:# prevents the game from being resumed when player has no lives left
		get_tree().paused = false# unpauses the game
	self.get_node("AnimatedSprite2D/Harm Zone").monitoring = false# prevents boss from hurting player after quiz is answered
	if player_node:
